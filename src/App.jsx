...
export default function Roadmap() {
  ...
  // Load saved checked keys on mount
  useEffect(() => {
    try {
      const raw = localStorage.getItem(STORAGE_KEY);
      if (raw) {
        const ids = JSON.parse(raw); // array of key strings, e.g. "0-2-1"
        const obj = {};
        ids.forEach((k) => { obj[k] = true; });
        setChecked(obj);
      }
    } catch (e) {
      // localStorage unavailable or corrupted value — start fresh
    } finally {
      setLoaded(true);
    }
  }, []);

  // Persist whenever checked changes (after initial load, so we don't overwrite with {})
  useEffect(() => {
    if (!loaded) return;
    try {
